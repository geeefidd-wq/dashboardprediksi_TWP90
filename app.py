...
FRIENDLY_LABELS = {
    "Outstanding pembiayaan (miliar RP)": "Outstanding pembiayaan",
...
COLUMN_HELP = {
    "Outstanding pembiayaan (miliar RP)": "Isi sesuai satuan historis model, yaitu miliar rupiah.",
...
def _update_sarimax_with_actual(sarimax_model, actual_value, X_row):
    """
    Update state SARIMAX secara aman untuk berbagai versi pmdarima/statsmodels.

    Beberapa artifact pmdarima lama dapat memanggil parameter internal
    error_action yang sudah tidak didukung pada versi library baru sehingga
    menyebabkan:
    Unknown keyword arguments: ['error_action'].

    Deployment prediction tetap berjalan dengan model artifact tanpa memaksa
    ...
    for kwargs in attempts:
        try:
            sarimax_model.update(y, **kwargs)
            return sarimax_model
        except TypeError as exc:
            if "error_action" in str(exc):
                continue
            continue
        except Exception:
            continue

    # Jika update gagal karena incompatibility library, gunakan model existing.
    return sarimax_model
